  ...
  return (
    <section className="figma-admin-page figma-profile-media-management-page" data-page="profile-media-management" data-design-reference="admin-media-library-v4" data-route={model.route.path} data-generated={model.generatedFrom} data-prototype-file={model.route.prototypeFile} data-auth-required={model.auth.role}>
        ...
        <div className="media-table" role="table" aria-label="Media library metadata">
          ...
          {sections.mediaLibrary.items.map((row) => (
            <div key={row.id} role="row" className="media-row" data-media-id={row.id} data-publish-ready={row.publishReady}>
              <span role="cell"><strong>{row.title}</strong><small>{row.url}</small></span><span role="cell">{row.altText}</span>
      ...
      <section className="figma-admin-state-grid" data-section="profile-media-states" data-state-note="media-uploading" data-state-note-secondary="media-missing-metadata">
        <h2>Profile / media states</h2>{sections.states.items.map((state) => <article key={state} data-state={state}>{state}</article>)}
        <p>{sections.states.metadataCopy}</p><a href={sections.states.permissionHref}>Return to admin login</a>
      </section>
    </section>
  );
}
