  ...
  return (
    <article className="figma-article-detail" data-page="article-detail" data-design-reference="article-detail-v4" data-generated={model.generatedFrom} data-slug={article.slug} data-prototype-file={model.route.prototypeFile}>
      ...
      <section data-section="comments" className="figma-conversation-section reader-section reader-section--comments" aria-labelledby="article-comments-heading">
        <div className="section-heading-row">
          <h2 id="article-comments-heading">Field Comments</h2>
          <span>{model.comments.length} verified signals</span>
        </div>
        <div className="reader-section__layout">
          <div className="reader-section__stream">
            {model.comments.map((comment) => (
              <article key={comment.id} className="comment" data-comment={comment.id}>
                <header><h3>{comment.name}</h3>{comment.createdAt ? <time dateTime={comment.createdAt}>{formatArticleDate(comment.createdAt)}</time> : <span>Verified reader</span>}</header>
                <p>{comment.body}</p>
              </article>
            ))}
            {model.comments.length === 0 ? <p className="reader-empty-state">No verified signals yet. Start the transmission.</p> : null}
          </div>
          ...
          <textarea id="comment-body" name="body" rows="5" placeholder="Leave a field note..." required />
          <p>Signals appear only after verification.</p>
          <button type="submit" disabled={commentStatus === "submitting"}>
            {commentStatus === "submitting" ? "Transmitting..." : "Send signal"}
          </button>
        </form>
        </div>
      </section>
      ...
      <section data-section="reviews" className="figma-conversation-section reader-section reader-section--reviews" aria-labelledby="article-reviews-heading">
        <div className="section-heading-row">
          ...
          <span>{model.reviews.length} verified ratings</span>
        </div>
        <div className="reader-section__layout">
          <div className="reader-section__stream">
            {model.reviews.length === 0 ? <p className="reader-empty-state">No verified ratings yet. Be the first to log one.</p> : null}
            {model.reviews.map((review) => (
              <article key={review.id} className="review" data-review={review.id} data-rating={review.rating}>
                <header><h3>{review.name}</h3>{review.createdAt ? <time dateTime={review.createdAt}>{formatArticleDate(review.createdAt)}</time> : <span>Reader rating</span>}</header>
          ...
          <textarea id="review-body" name="body" rows="5" placeholder="Your read on the drop..." required />
          <p>Reviews appear only after verification.</p>
          <button type="submit" disabled={reviewStatus === "submitting"}>
            {reviewStatus === "submitting" ? "Logging..." : "Log review"}
          </button>
        </form>
        </div>
      </section>
